...
def check_plagiarism_online(input_text):
    """
    Checks for plagiarism using the Google Custom Search API.
    Ensures input validity and robust error handling.
    """
    api_key = 'AIzaSyCqvaeUTAlnmS6OE07aofH7a5VvfWwXu2U'  # Replace with your actual API key
    engine_id = '46e309481c1b4431f'  # Replace with your engine ID

    # Clean the input text before using it in the API
    input_text = clean_text(input_text)

    if not input_text.strip():
        print("Error: Input text is empty or invalid after cleaning.")
        return {
            'copied': False,
            'sources': [],
            'percentage_copied': 0,
    ...
    try:
        ...
        print(f"Searching for: {input_text[:100]}...")
        ...
        print(f"Unexpected error during plagiarism check: {e}")
        return {
            'copied': False,
            'sources': [],
            'percentage_copied': 0,
